...
public class NormalMapWindow extends PlatformWindow implements SVIAnimationListener{
	
	public static final float SENSOR_SCALE = 0.995f;
	public static final int BUTTON_ANIMATION_DURATION = 100;
	public static final SVIColor 	BUTTON_ON_COLOR = new SVIColor(1.0f,0.6f,0.55f,1.0f);
	public static final String TAG = "NormalMapWindow";
	...
	private SVIImage mSpecularMapImage = null;
	private Bitmap	mSpecularMapBitmap = null;	
	...
	public NormalMapWindow(Context context){
		super(context);
		...
		is = activity.getResources().openRawResource(R.drawable.specular_test);
		mSpecularMapBitmap = BitmapFactory.decodeStream(is);
		mSpecularMapImage = new SVIImage();
		mSpecularMapImage.setBitmap(mSpecularMapBitmap);
		...
		if( event.getAction() == MotionEvent.ACTION_DOWN){
			for (int n = 0; n < 15; n++){
				...
				if (Region.checkHit(new SVIPoint(event.getX(), event.getY()))){
					...
					//dropPostRipple(event.getX(),event.getY(),0.7f);
					break;
				}
			}	
	...
	protected void buildSlideTree(){
		...
		for (int y = 0; y < 5; y++){
			for (int x = 0; x < 3; x++){
				...
				//mButtonSlide[n].setNormalImage(mNormalMapImage);
				//mButtonSlide[n].setSpecularImage(mSpecularMapImage);
